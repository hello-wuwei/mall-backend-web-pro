...
const SkuManage = ({ match, location }:RouteComponentProps) => {
  const { params, setParams } = useParams(clearEmptyValue({ status: pathKeyMap[location.pathname] ? pathKeyMap[location.pathname] : '' }))

  const onGoodsChange = (key:string) => {
    setParams({ status: pathKeyMap[key] ? pathKeyMap[key] : '' })
    history.push(key)
  }
    ...
    <div>
      ...
      <div className="go-page-body">
        <GoTabBar defaultActiveKey={location.pathname} onChange={onGoodsChange}>
          ...
          <GoTabBar.Item key={`${match.url}/onsale`} value={`${match.url}/onsale`}>上架</GoTabBar.Item>
          <GoTabBar.Item key={`${match.url}/unsale`} value={`${match.url}/unsale`}>下架</GoTabBar.Item>
          <GoTabBar.Item key={`${match.url}/noshow`} value={`${match.url}/noshow`}>不再展示</GoTabBar.Item>
        </GoTabBar>
        <div style={{ marginTop: 20 }}>
          <Switch>
            ...
            <WrapRoute path={`${match.url}/onsale`} component={() => <OnSale getList={getSkus} rowKey="skuCode" dataSource={list} pagination={pagination} />} />
            <WrapRoute path={`${match.url}/unsale`} component={() => <UnSale getList={getSkus} rowKey="skuCode" dataSource={list} pagination={pagination} />} />
            <WrapRoute path={`${match.url}/noshow`} component={() => <NoShow getList={getSkus} rowKey="skuCode" dataSource={list} pagination={pagination} />} />
          </Switch>
        </div>
      </div>
    </div>
